...
def load_dataset(
    ...
    if dataset_name == "msmarco" and data_split == "gen_qs":
        ...
        # build hf dataset
        qp_pairs_dataset = Dataset.from_dict(
            {"query": queries, "query_id": list(range(len(queries)))}
        )
    ...
    corpus = {
        pid: {"text": (content["title"] + " " + content["text"]).strip()}
    ...
    if filter_concepts and not filter_in_qids:
        ...
        concept_filenames = [
            f for f in os.listdir("config/cover_alg") if f.startswith("concept-")
        ]
        ...
        logger.info(
            f"Filtered concepts: {len(concept_filenames)=}, {len(queries)=}, {len(qrels)=}"
        )

    # [OPTIONAL] Filter only specific qids
    if filter_in_qids:
        filtered_queries = {
            qid: q for qid, q in queries.items() if qid in filter_in_qids
        }
        ...
        logger.info(
            f"Filtered specific qids: {filter_in_qids=}, {len(queries)=}, {len(qrels)=}"
        )
    ...
    if data_portion < 1.0:
        ...
        qrels = {
            qid: qrel
            for i, (qid, qrel) in enumerate(qrels.items())
            if qid in queries.keys()
        }

    logger.info(
        f"Loaded data: {dataset_name=}, {len(corpus)=}, {len(queries)=}, {len(qrels)=}"
    )
...
def _build_hf_dataset(corpus, queries, qrels) -> Dataset:
    ...
    qp_pairs_dataset = Dataset.from_dict(
        {"query": q_texts, "query_id": q_ids, "passage": p_texts, "pid": p_ids}
    )
